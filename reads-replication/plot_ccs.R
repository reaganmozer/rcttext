## Function for plotting the results of a CCS model or set of CCS models

setwd(dirname(rstudioapi::getActiveDocumentContext()$path))
setwd("../")
options(stringsAsFactors = FALSE)

plot.ccs = function(out, main, range = NULL, xadj=c(-0.025,0.025)){
  out1 = out
  if ( is.null( range ) ) {
  ...
  plot(x=xvals,y=yvals, xlim=xlim,ylim=c(0,10),ylab="",
       yaxt="n",xlab="",xaxt="n",main=main, cex.main=1.1,
       type="n")
  abline(v=0,lty=2)
